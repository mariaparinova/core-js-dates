...
function getNextFriday(date) {
  ...
  const friday = 5;
  let nextFridayIn = friday - dateObj.getDay();
  ...
  dateObj.setDate(dateObj.getDate() + nextFridayIn);
...
function getCountDaysInMonth(month, year) {
  const tailMonth = [31, 30, 29];

  for (let i = 0; i < tailMonth.length; i += 1) {
    const date = new Date(year, month - 1, tailMonth[i]);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return (
    Math.abs(new Date(dateStart) - new Date(dateEnd)) / 1000 / 60 / 60 / 24 + 1
  );
}
...
function formatDate(date) {
  ...
  let [, month, day] = date.split('T')[0].split('-');
  let [hour, , sec] = date.split('T')[1].split(':');
  const [, min] = date.split('T')[1].split(':');
  const initHour = hour;
  ...
  if (hour.length === 2 && hour.at(0) === '0') {
    hour = hour.at(1);
  }

  sec = sec.slice(0, 2);

  return `${month}/${day}/${year}, ${hour}:${min}:${sec} ${initHour > 11 ? 'PM' : 'AM'}`;
}
...
function getWeekNumberByDate(date) {
  ...
  let dayFromTable = ordinalDayTable[month];

  if (dayFromTable > 1 && isLeap) {
    dayFromTable += 1;
  }

  const dayOfYear = dayFromTable + dayOfMonth;
...
function getNextFridayThe13th(date) {
  const nextFridayObj = getNextFriday(date);
  const day = nextFridayObj.getDate();

  return day === 13 ? nextFridayObj : getNextFridayThe13th(nextFridayObj);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const [startDD, startMM, startYYYY] = period.start.split('-');
  const [endDD, endMM, endYYYY] = period.end.split('-');
  const start = +new Date(`${startYYYY}-${startMM}-${startDD}`);
  const end = +new Date(`${endYYYY}-${endMM}-${endDD}`);
  let day = start;
